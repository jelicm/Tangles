...
async function reloadDiv() {
    try {
        const response = await fetch('https://uhhgpt.uni-hamburg.de');
...
async function executeSequentially() {
  ...
  for (let i = 1; i <= 5; i++) {
    inputField1 = document.querySelector(".input-field");
    inputField1.value = "answer this questions in csv format with only one row with answers to this questions for " + dogBreeds[i%50] + " and without any additional text, here are the questions:" + questionnaire
    inputField1.dispatchEvent(new Event('input'));
    let rez = await request();
    console.log(rez)
    await delay(10000);
    console.log('10 seconds have passed!');
    let lastMessage = document.querySelector(".message:last-child");
    let messageTextElement = lastMessage.querySelector(".message-text"); 
    //console.log(messageTextElement.textContent);
    replays0.push(messageTextElement.textContent)
    await delay(2000);
    await reloadDiv()
...
executeSequentially().then(rez => {
  const csvContent = replays0.join("\n");
